                            ...
                            <div class="row">
                                ...
                                <div class="col-md-3">
                                    <div class="form-group">
                                        ...
                                        <input type="text" value="{{ $data_inicio ?? '' }}" name="data_inicio"
                                ...
                                <div class="col-md-3">
                                    <div class="form-group">
                                        ...
                                        <input type="text" value="{{ $data_fim ?? '' }}" name="data_fim"
                                            class="form-control datepicker" />
                                    </div>
                                </div>
                    ...
                    @if ($coordenador !== 1)
                        <div class="box-header">
                            <div class="col-md-6" style="padding-left: 0;">
                                <h3 class="box-title">Registros de: <b>{{ $data_inicio ?? '' }} a {{ $data_fim ?? '' }}</b>
                                </h3>
                            </div>
                        </div>
                    @endif
                                ...
                                @foreach ($registros as $registro)
                                    ...
                                    @if ($registro_dia != $registro->data and $registro_dia != '')
                                        ...
                                        <tr>
                                            ...
                                            <td><?= $horas_trabalhadas_total_dia_h ?>:<?= $horas_trabalhadas_total_dia_m ?>
                                            </td>
                                        </tr>
                                    @endif
                                    <tr>
                                        ...
                                        <td>
                                            @if ($registro->entrada_status === null)
                                                <a href="#modal-correcao-{{ $registro->id }}" data-toggle="modal"
                                                    onclick="setaDadosModal({{ $registro->id }}, 'entrada', '<?= $varData2 ?>', '')"><i
                                                        class="fas fa-exclamation-triangle text-yellow"></i></a>
                                            @else
                                                <?php
                                                if ($registro->entrada_status == 0) {
                                                    $varCor = '#005599';
                                                }
                                                if ($registro->entrada_status == 1) {
                                                    $varCor = '#D39745';
                                                }
                                                if ($registro->entrada_status == 2) {
                                                    $varCor = '#67b021';
                                                }
                                                ?>
                                                @if ($registro->entrada_status != 0)
                                                    <span
                                                        style="color: <?= $varCor ?>;">{{ substr($registro->entrada, 0, 5) }}</span>
                                                @else
                                                    <a href="#modal-correcao-{{ $registro->id }}"
                                                        style="color: <?= $varCor ?>;" data-toggle="modal"
                                                        onclick="setaDadosModal({{ $registro->id }}, 'saida', '<?= $varData2 ?>', '<?= substr($registro->entrada, 0, 5) ?>')">{{ substr($registro->entrada, 0, 5) }}</a>
                                                @endif
                                            @endif
                                        </td>
                                        <td>
                                            @if ($registro->saida_status === null)
                                                <a href="#modal-correcao-{{ $registro->id }}" data-toggle="modal"
                                                    onclick="setaDadosModal({{ $registro->id }}, 'saida', '<?= $varData2 ?>', '')"><i
                                                        class="fas fa-exclamation-triangle text-yellow"></i></a>
                                            @else
                                                <?php
                                                ...
                                                if ($registro->saida_status == 1) {
                                                    $varCor = '#D39745';
                                                }
                                                if ($registro->saida_status == 2) {
                                                    $varCor = '#67b021';
                                                }
                                                ?>
                                                @if ($registro->saida_status != 0)
                                                    <span
                                                        style="color: <?= $varCor ?>;">{{ substr($registro->saida, 0, 5) }}</span>
                                                @else
                                                    <a href="#modal-correcao-{{ $registro->id }}"
                                                        style="color: <?= $varCor ?>;" data-toggle="modal"
                                                        onclick="setaDadosModal({{ $registro->id }}, 'saida', '<?= $varData2 ?>', '<?= substr($registro->saida, 0, 5) ?>')">{{ substr($registro->saida, 0, 5) }}</a>
                                                @endif
                                            @endif
                                        </td>
                                    ...
                                    @if ($conta_registro == $total_registro)
                                        ...
                                        ?>
                                        <tr>
                                            <td colspan="4" align='right'><b>Total trabalhado no período de <span
                                                        style="color: #900;"><?= $data_inicio ?? '' ?></span> a <span
                                                        style="color: #900;"><?= $data_fim ?></span>:</b></td>
                                            <td><b><?= $horas_trabalhadas_total_h ?>:<?= $horas_trabalhadas_total_m ?></b>
                                            </td>
                                        </tr>
                                    @endif
                                    ...
                                    <div id="modal-correcao-{{ $registro->id }}" class="modal fade" tabindex="-1"
                                        ...
                                        <div class="modal-dialog modal-md">
                                            <div class="modal-content">
                                                ...
                                                <form method="post" action="/painel/ponto/salvar"
                                                    ...
                                                    <div class="modal-body">
                                                        ...
                                                        <div class="row form-group">
                                                            ...
                                                            <div class="col-md-3">
                                                                <input type="radio" name="tipo" class="tipo-ajuste"
                                                                    id="tipo-entrada-{{ $registro->id }}"
                                                                    data-id='<?= $registro->id ?>' value="entrada"> Entrada
                                                            </div>

                                                            <div class="col-md-3">
                                                                <input type="radio" name="tipo" class="tipo-ajuste"
                                                                    id="tipo-saida-{{ $registro->id }}"
                                                                    data-id='<?= $registro->id ?>' value="saida"> Saída
                                                            </div>
                                                        ...
                                                        <div class="row form-group">
                                                            <div class="col-md-4">
                                                                <input type="text" name="data"
                                                                    id="data-{{ $registro->id }}" class="form-control"
                                                                    placeholder="Data" value="" required>
                                                            </div>
                                                        </div>

                                                        <div class="row form-group"
                                                            id="hora-simples-{{ $registro->id }}">
                                                            <div class="col-md-4">
                                                                <input type="text" name="hora"
                                                                    id="hora-{{ $registro->id }}"
                                                                    class="form-control time" placeholder="Hora"
                                                                    value="">
                                                            </div>
                                                        </div>

                                                        <div class="row form-group" id="hora-periodo-{{ $registro->id }}"
                                                            style="display: none;">
                                                            <div class="col-md-4">
                                                                <input type="text" name="hora_entrada"
                                                                    id="hora_entrada-{{ $registro->id }}"
                                                                    class="form-control" placeholder="Hora de Entrada"
                                                                    value="">
                                                            </div>
                                                            <div class="col-md-4">
                                                                <input type="text" name="hora_saida"
                                                                    id="hora_saida-{{ $registro->id }}"
                                                                    class="form-control" placeholder="Hora de Saída"
                                                                    value="">
                                                            </div>
                                                        </div>
                                                            ...
                                                            <select name="justificativa" class="form-control" required>
                                                                ...
                                                                @foreach ($justificativas as $justificativa)
                                                                    <option value="{{ $justificativa->id }}">
                                                                        {{ $justificativa->descricao }}</option>
                                                                @endforeach
                                                            </select>
                                                        </div>
